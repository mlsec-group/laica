...
from .utils import format, encode_single
from ..core.corpus import MemoryCorpus, Instruction2VecCorpus, ParallelCorpus, FileCacheCorpus
...
def embed(path, data, out):
    logger.info("Loading Word2Vec model.")
    model = Word2Vec.load(path)

    for ops in data:
        low_pc, ops = ops.strip().split(": ")
        low_pc = int(low_pc)
        encoded_insns = []
        ...
        out.write(','.join(map(lambda vector_str: f"[{vector_str}]", encoded_insns)))
        out.write("\n")
...
def stream(pkg, out, format_):
    for ops, low_pc in iter_func_ops_from_deb(pkg):
        if format_:
            ...
            insns = ['|'.join(format(insn)) for insn in insns]
...
@click.option("--vectorsize", "vector_size", default=8)
...
def train(pkgs, savepath, workers, cachedir, *args, **kwargs):
    """Train a Instruction2Vec-Model using debian packages as corpus.

    Train a Instruction2Vec-Model using binary code from the debian
    ...
    if cachedir:
        dataset = MemoryCorpus(ParallelCorpus(
            [FileCacheCorpus(Instruction2VecCorpus(pkg), cachefile=cachedir / pkg2cachefile(pkg)) for pkg in pkgs],
            n_jobs=workers))
    else:
        dataset = MemoryCorpus(ParallelCorpus([Instruction2VecCorpus(pkg) for pkg in pkgs], n_jobs=workers))
    callbacks = [EpochLogger()]
    logger.info(f"Training Word2Vec model on {len(pkgs)} debian packages.")
    model = Word2Vec(dataset, compute_loss=True, callbacks=callbacks, window=128, epochs=10, min_count=1,
                     sg=1, negative=5, alpha=1e-3, min_alpha=1e-3, workers=workers, *args, **kwargs)
...
def iter_corpus(pkgs):
    pkgs = list(collect_packages(pkgs))
    dataset = itertools.chain(*(Instruction2VecCorpus(pkg) for pkg in pkgs))
